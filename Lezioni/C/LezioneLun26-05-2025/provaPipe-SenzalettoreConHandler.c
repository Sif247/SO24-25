/* Programma in C provaPipe-SenzalettoreConHandler.c in cui, diversamente dal programma provaPipe-Senzalettore.c, il figlio AGGANCIA una funzione per trattare il segnale SIGPIPE e quindi termina in modo normale */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <fcntl.h>
...
void Azione(int sig)
{ 	printf("DEBUG-Figlio %d: Arrivato segnale # %d\n", getpid(), sig);
	flag = TRUE;
}
...
{   
	...
	{   
		...
		sleep(5); 	/* inseriamo questa sleep cosi' da essere sicuri che quando il figlio tenta di scrivere, il padre sia gia' morto ed inoltre per provare ad eseguire il comando ps -lf! */
		...
		while (read(fd, mess, MSGSIZE)) /* il contenuto del file e' tale che in mess ci saranno 4 caratteri e il terminatore di linea */
		{
			...
			{ 	printf("NON C'E' LETTORE\n"); 		/* quando la variabile flag diventa TRUE allora il processo esce MA NON in modo anomalo, ma normale con una exit che pero' segnalerebbe un errore */
		 		exit(-1); /* in realta' il padre NON aspetta il figlio! */
			}
    			j++;
		}
