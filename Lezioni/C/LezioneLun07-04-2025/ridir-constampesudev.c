/* Programma in C ridir-constampesudev.c che mostra in piu' rispetto al programma ridir.c i valori delle varibili infile e outfile, sul dispositivo /dev/tty, oltre che gli errori dopo aver chiuso lo standard output */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <fcntl.h>
...
{    int infile, outfile, nread;	/* usati per open e valore ritorno read */
     ...
     int fd; 				/* usato per dispositivo /dev/tty */
     char sErroreCreat[]= "Errore nella creazione file\n";
     char sErroreWrite[]= "Errore nella scrittura del file\n";
     char s[100] = "DEBUG-Valore di infile ";	/* stringa usata nella funzione sprintf */
	...
	/* per prima cosa apriamo in scrittura /dev/tty */
	...
	{ 	write(fd, sErroreCreat, strlen(sErroreCreat));	/* questa volta scriviamo su fd e quindi su /dev/tty invece che su standard error */
		exit(4);
	}
	...
	{ 
		...
		{ 	write(fd, sErroreWrite, strlen(sErroreWrite));	/* questa volta scriviamo su fd e quindi su /dev/tty invece che su standard error */
			exit(5);
		}
	}	
