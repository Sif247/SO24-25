/* File parametri2.c che deve eseguire una serie di controlli sui 3 parametri passati: nome file esistente, numero intero strettamente positivo e singolo carattere; quindi deve stampare il nome dell'eseguibile, il numero di parametri e ogni singolo parametro */
#include <stdio.h>
#include <stdlib.h>
#include <fcntl.h>
#include <string.h>
...
{ 	
   ...
   char C; 	/* serve per selezionare il terzo parametro */
	...
	if ((fd = open(argv[1], O_RDONLY)) < 0)
	{
		...
		exit(2); 	/* come in SHELL sempre valori di ritorno differenti! */
	}

	/* controllo secondo parametro: usiamo la funzione di libreria atoi: se si riesce a convertire in un numero strettamente positivo lo consideriamo un controllo sufficiente */
	N=atoi(argv[2]);	/* nome della variabile definito nel testo */	
	if (N <= 0) 
	{ 	
	...
	/* controllo terzo parametro: usiamo la funzione di libreria strlen */
