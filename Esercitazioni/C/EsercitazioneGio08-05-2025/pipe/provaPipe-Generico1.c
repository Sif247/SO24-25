/* Programma in C provaPipe-Generico1.c che diversamente dal programma provaPipe.c usa un protocollo di comunicazione diverso: il figlio deve leggere ogni linea (composta di un numero qualunque di caratteri) e quindi deve mandare al padre la lunghezza della linea e quindi la stringa corrispondente a tale linea (usando la stessa pipe); il padre nel ciclo di lettura, per prima cosa, legge la lunghezza corrente e quindi usa tale informazione per leggere la stringa */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <fcntl.h>
...
{  
   ...
   int pidFiglio, status, ritorno; 	/* per wait padre */
			...
			if (mess[L] == '\n') /* siamo arrivati alla fine di una linea */
        		{	
 				/* il padre ha concordato con il figlio che gli mandera' solo stringhe e quindi dobbiamo sostituire il terminatore di linea con il terminatore di stringa */
    				mess[L]='\0';
 				L++; /* incrementiamo L per tenere conto anche del terminatore di linea; in alternativa, dato che ora mess e' una stringa si potrebbe usare la funzione strlen, ma avendo L e' INUTILE! */
      				/* comunichiamo L al processo padre */
      				write(piped[1],&L,sizeof(L));	/* L e' un int */
	...
	{ 	
		/* ricevuta la lunghezza, il padre puo' andare a leggere la stringa */
