/* Programma in C provaEsame1.c che ha bisogno di almeno due nomi di file, esistenti e leggibili */
#include <stdio.h>
#include <stdlib.h>
#include <fcntl.h>
#include <unistd.h>
...
int main(int argc, char *argv[])
{
   ...
   int pidFiglio, status, ritorno;    /* variabili per wait */
  	...
  	/* controllo sul numero di parametri: almeno due nomi file ==> controllo lasco! */
  	if (argc < 3) 	
  	{ 	printf("Errore: Necessari almeno 2 parametri (nomi di file) per %s e invece argc = %d\n", argv[0], argc);
    		exit(1);
  	}
  	...
  	for (n=0; n < N; n++)
  	{
			...
			*/
			...
			/* chiusura lati pipe non utilizzati */
      			...
      			close(p[1][0]);
      			close(p[n % 2][1]); /* chiude il lato di scrittura della pipe non usata: quindi se i e' pari si chiude il lato di scrittura di p[0], altrimenti se i e' dispari si chiude quello di p[1] */
      			/* apertura del file associato */
      			if ((fdr = open(argv[n+1], O_RDONLY)) < 0) /* il processo di indice n deve aprire il file di indice n+1 */
			/* N.B. i processi di indice pari (ad esempio 0) leggono i file di indice dispari (ad esempio argv[1]) e quindi devono selezionare i caratteri alfabetici, mentre i processi di indice dispari (ad esempio 1) leggono i file di indice pari (ad esempio argv[2]) e quindi devono selezionare i caratteri numerici */ 
      			...
      			{
        			if ((((n % 2) == 0) && isalpha(ch)) || 		/* il file dispari e' letto in realta' da un processo di indice pari e quindi si controlla che il carattere letto sia un alfabetico */
            				(((n % 2) == 1) && isdigit(ch)))   	/* il file pari e' letto in realta' da un processo di indice dispari e quindi si controlla che il carattere letto sia un numerico */
