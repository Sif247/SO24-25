/* Programma in C myGrepConFork-ridStError.c che ha bisogno di DUE parametri (la stringa da cercare e il nome del file) e mostra l'uso della primitiva execlp per la esecuzione del comando grep da parte di un processo figlio, oltre che l'uso delle ridirezione su /dev/null (come fatto nella SHELL), in questo caso sia dello standard output che dello standard output, per avere solo il risultato di successo/insuccesso del comando grep! */
#include <stdio.h>
#include <unistd.h>
#include <stdlib.h>
#include <fcntl.h>
