...
*) echo Errore: voglio due parametri, mentre ne hai passati $#! Uso: append dadove sucosa	#casi NON giusti
   exit 1;;
esac

#se arriviamo qui siamo sicuri che siano stati passati esattamente due parametri
...
do
	...
	#N.B. Inserire sempre il parametro in modo da verificare l'errore!
   	*)   echo DEBUG-OK nome $i ;;				#caso giusto
   	esac
done

#se arriviamo qui siamo sicuri che siano stati passati esattamente due parametri in forma relativa semplice
...
else	#file esiste e controlliamo se scrivibile
	if test ! -w $2
